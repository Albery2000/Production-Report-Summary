...
def extract_wells_with_net_diff_bo(file_content):
    ...
    try:
        ...
        # Extract wells with ZERO Net BO for the new table
        zero_net_bo_df = df_before_total[
            (df_before_total[net_bo_col].notna()) & 
            (df_before_total[net_bo_col] == 0)
        ].copy()
        ...
        # Also clean zero Net BO dataframe
        if not zero_net_bo_df.empty:
            zero_mask = ~zero_net_bo_df[well_name_col].isin(field_names)
            zero_net_bo_df = zero_net_bo_df[zero_mask].copy()
            zero_net_bo_df = zero_net_bo_df[zero_net_bo_df[well_name_col].notna()]
            zero_net_bo_df = zero_net_bo_df[zero_net_bo_df[well_name_col] != '']
            zero_net_bo_df = zero_net_bo_df.reset_index(drop=True)
...
def create_zero_net_bo_table(zero_net_bo_df, original_columns):
    ...
    try:
        ...
        display_columns = [field_col, well_name_col, net_bo_col, net_diff_bo_col]
